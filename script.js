    ...
    (function setupCapsuleSpinOnScroll() {
        ...
        function applySpin(p) {
            ...
            if (photoEl) {
                ...
                // Mantém a centralização (translate) adicionando a rotação
                const isMobile = window.innerWidth <= 900;
                if (isMobile) {
                    // Mobile usa posição absoluta com translate para centralizar
                    photoEl.style.transform = `translate(-50%, -46%) rotate(${deg}deg)`;
                } else {
                    // Desktop: flexbox centraliza, só precisa de rotação
                    photoEl.style.transform = `rotate(${deg}deg)`;
                }
            } else {
        ...
        // Sempre adiciona listeners nativos para garantir funcionamento no mobile
        window.addEventListener('scroll', onScrollSpin, { passive: true });
        window.addEventListener('touchmove', onScrollSpin, { passive: true });
    ...
    (function setupCapsuleCtaTrigger() {
        ...
        const trySetup = () => {
            ...
            try {
                ...
                if (overlays && overlays.length) {
                    overlays.forEach(img => {
                        const baseTransition = isMobileView ? 'opacity .24s ease, transform .24s ease' : 'opacity .28s ease, transform .28s ease';
                        img.style.transition = baseTransition;
                        img.style.opacity = '0';
                        img.style.visibility = 'hidden';
                        // for mobile start slightly offset so the overlay can translate into place
                        if (isMobileView) img.style.transform = 'translateY(10px)';
                    });
                }
            } catch (e) { /* silent */ }
                                ...
                                requestAnimationFrame(() => {
                                    overlays.forEach(img => {
                                        try {
                                            ...
                                            // translate into place (handles mobile starting offset)
                                            img.style.transform = 'translateY(0)';
                                            // explicitly trigger fade-in by setting opacity to 1 (initial opacity should be 0)
                                            img.style.opacity = '1';
                                        } catch (e) { /* silent */ }
                                    });
                                    // cleanup: remove inline transitions/transforms after animation finishes so CSS can take over
                                    setTimeout(() => {
                                        overlays.forEach(img => {
                                            try {
