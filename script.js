// --- CONTROLE DE VISIBILIDADE DO HEADER E TOGGLE ---
(function () {
    'use strict';

    // Esconde header e toggle IMEDIATAMENTE (antes de qualquer render)
    const hideInitialStyles = document.createElement('style');
    hideInitialStyles.id = 'sopy-header-toggle-hide';
    hideInitialStyles.textContent = `
        .js-head-main, .head-main { 
            transform: translateY(-100%) !important; 
            opacity: 0 !important;
            transition: transform 0.35s ease, opacity 0.25s ease !important;
        }
        .pod-toggle-label, .product-toggle-container {
            opacity: 0 !important;
            pointer-events: none !important;
            transition: opacity 0.3s ease !important;
        }
        .js-head-main.header-visible, .head-main.header-visible {
            transform: translateY(0) !important;
            opacity: 1 !important;
        }
        .pod-toggle-label.toggle-visible, .product-toggle-container.toggle-visible {
            opacity: 1 !important;
            pointer-events: auto !important;
        }
    `;
    document.head.appendChild(hideInitialStyles);

    // Inicializa controle após DOM carregar
    function initHeaderToggleControl() {
        const header = document.querySelector('.js-head-main, .head-main');
        const toggle = document.querySelector('.pod-toggle-label, .product-toggle-container');
        ...
        // Referência para a 2ª seção (capsula-3d ou a que vier depois do hero)
        const secondSection = document.querySelector('#capsula-3d, .sopy-capsule3d, .sopy-main section:nth-child(2)');
        const secondSectionTop = secondSection ? secondSection.offsetTop : window.innerHeight;

        function updateVisibility() {
            const currentScrollY = window.scrollY || window.pageYOffset;
            const scrollingUp = currentScrollY < lastScrollY;
            const scrolledPast100px = currentScrollY > 100;

            // HEADER: só aparece se estiver scrollando para CIMA e já passou de 100px
            if (scrollingUp && scrolledPast100px) {
                header.classList.add('header-visible');
            } else if (!scrollingUp && scrolledPast100px) {
                header.classList.remove('header-visible');
            } else if (currentScrollY <= 100) {
                // No topo, esconde o header
                header.classList.remove('header-visible');
            }

            // TOGGLE: só aparece a partir da 2ª seção
            if (toggle) {
                if (currentScrollY >= secondSectionTop - 100) {
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initHeaderToggleControl);
    } else {
        initHeaderToggleControl();
    }
})();
